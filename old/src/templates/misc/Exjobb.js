    ...
    <div className="Home">
      ...
      <div className="Middle">
        ...
        {exjobbs.map((exjobb, index) => (
          <PostPreview
            key={index}
            title={exjobb.attributes.title}
            date={exjobb.attributes.publishedAt}
            content={exjobb.attributes.preview_content}
            slug={"/exjobb/" + exjobb.attributes.slug}
            thumbnailURL={
              exjobb.attributes.thumbnail.data !== null
                ? exjobb.attributes.thumbnail.data.attributes.url
                : "public/logo_white.png"
            }
            thumbnailAlt={
              exjobb.attributes.thumbnail.data !== null
                ? exjobb.attributes.thumbnail.data.attributes.alternativeText
                : "Logo"
            }
            thumbnailWidth={
              exjobb.attributes.thumbnail.data !== null
                ? exjobb.attributes.thumbnail.data.attributes.width
                : 100
            }
            thumbnailHeight={
              exjobb.attributes.thumbnail.data !== null
                ? exjobb.attributes.thumbnail.data.attributes.height
                : 100
            }
            id={exjobb.id}
          />
        ...
        {exjobb && (
          <div className="PostPreview">
            ...
            <div className="PostPreview-Date">
              {showDatetime(exjobb.attributes.publishedAt)}
            </div>
            <div className="PostPreview-Content">
              <Markdown remarkPlugins={[remarkGfm]}>
                {exjobb.attributes.content}
              </Markdown>
            </div>
          </div>
        )}
